...
def _ready_user_tasks(task_info: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        if task.get("state") == "READY" and task.get("typename") in _ACTIONABLE_TASK_TYPES
    ]
...
async def _instance_ready_tasks(process_instance_id: int, token: str) -> list[dict[str, Any]]:
    ...
    if not isinstance(tasks, list):
        tasks = []
    return _ready_user_tasks(tasks)
    ...
    async def list_tasks(
        ...
        try:
            ...
            if process_instance_id is not None:
                ...
                return {"results": results, "pagination": {"count": len(results), "total": len(results)}}
